...
Eigen::Matrix2d pair_stiffness_integral(const Element &el, const U &u, const V &v, double lambda, double mu)
{
...
void impose_homogeneous_condition(
    const Mesh &mesh, StiffnessType<Mesh> &K, RHS &rhs, size_t which, double scale = 1.0)
{
...
void impose_dirichlet_condition(
    const Mesh &mesh, StiffnessType<Mesh> &K, RHS &rhs, size_t which, const Vector &value,
    double scale = 1.0)
{
    std::vector<size_t> adjacent;
    adjacent.reserve(2 * msh::max_node_adjacencies<Mesh>);
...
void add_point_forces(const Eigen::Matrix<T, 2, N, Options...> &force, const IndexContainer &nodes, RHS &F)
{
